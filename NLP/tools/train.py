...
if __name__ == "__main__":
    ...
    for i_run in range(conf['nb_runs']):
        ...
        if resv[mainIndex]>bestresult:
            bestresult = resv[mainIndex]
            bestwfname = wfname
    print()
    ...
    for split in [trainf, validf, testf]:
        ...
        for field,_ in res[split][0].items():
            values = [r[field] for r in res[split]]
            fres[split][field] = values
            mres[split][field] = np.mean(values)
            bres[split][field] = stat(flog, conf['nb_runs'], split, field, values)
    for field,_ in res[validf][0].items():
        if fres[trainf]:
            print('train-valid %s Pearsonr: %f' % (field, ss.pearsonr(fres[trainf][field], fres[validf][field])[0],))
            print('train-valid %s Pearsonr: %f' % (field, ss.pearsonr(fres[trainf][field], fres[validf][field])[0],), file=flog)
        if fres[testf]:
            print('valid-test %s Pearsonr: %f' % (field, ss.pearsonr(fres[validf][field], fres[testf][field])[0],))
            print('valid-test %s Pearsonr: %f' % (field, ss.pearsonr(fres[validf][field], fres[testf][field])[0],), file=flog)
    def res_columns(mres, pfx=' '):
        return('%s%.6f |%s%.6f |%s%.6f' % (pfx,mres[trainf][mainIndex], pfx,mres[validf][mainIndex], pfx,mres[testf].get(mainIndex, np.nan)))
    print('| % -24s |%s | %s' % (modname, res_columns(mres),
          '(defaults)' if not params else ' '.join(['``%s``' % (p,) for p in params if not p.startswith('vocabf=')])))
    print('| % -24s |%s |' % ('', res_columns(bres, '±')))
    print('| % -24s |%s | %s' % (modname, res_columns(mres),
          '(defaults)' if not params else ' '.join(['``%s``' % (p,) for p in params if not p.startswith('vocabf=')])), file=flog)
