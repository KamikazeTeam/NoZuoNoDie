...
from keras.layers import GlobalAveragePooling1D, Dense, Reshape
...
class CNNMod(AbstractMod):
    ...
    def create_model(self, mergedinp0, mergedinp1, N, c):
        ...
        for fl, cd in c['cdim'].items():
            ...
            cop.append(Convolution2D(name='cop%d'%(fl,), nb_filter=6, nb_row=c['spad'], nb_col=1, border_mode='valid',#valid same
                                     init=c['cinit'], W_regularizer=l2(c['cl2reg'])))#subsample_length=1))
            acp.append(Activation(c['cact']))
        ...
        def mergego(input00, input01, input10, input11, mode, flag):
            ...
            return output0, output1
        def shapego(input0, input1, flag):
            if not flag: return input0, input1
            output0,output1=[],[]
            for fl in range(len(c['cdim'])):
                output0.append(Reshape((c['spad'],N,1))(input0[fl]))
                output1.append(Reshape((c['spad'],N,1))(input1[fl]))
            return output0, output1
        input0=[]
        input1=[]
        for fl in range(len(c['cdim'])):
        ...
        self.testlayers.append( input0[0])
        ...
        if 0:
            ...
            self.testlayers.append(resnet0[0])
        else:
            resnet0, resnet1=batnor0, batnor1
        active0, active1=layergo(resnet0,resnet1, act, 1)
        drpout0, drpout1=layergo(active0,active1, dpt, 0)
        if 1:
            transf0, transf1=shapego(drpout0,drpout1,1)
            copool0, copool1=layergo(transf0,transf1, cop, 1)
            acpool0, acpool1=layergo(copool0,copool1, acp, 1)
            vpools0, vpools1=acpool0,acpool1
            self.testlayers.append(transf0[0])
            self.testlayers.append(copool0[0])
            self.testlayers.append(vpools0[0])
        else:
