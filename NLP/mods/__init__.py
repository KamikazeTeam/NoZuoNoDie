...
class AbstractMod():
    ...
    def scorer_multilayer(self, output_dim, tensor0, tensor1, N, c):
        ...
        mlpdiff = alldiff
        for i, D in enumerate(c['Ddim']):
            dendiff = Dense(name='mlp'+'hdn%d'%(i,), 
                            output_dim=int(N*D), 
                            W_regularizer=l2(c['Dl2reg']), 
                            activation=c['Dact'], 
                            init=c['Dinit'])(mlpdiff)
            mlpdiff = dendiff
    ...
    def scorer_multiperspective(self, output_dim, tensor0, tensor1, N, c):
        ...
        conf['dim'] 	= N
        conf['n_hidden']= 150
        conf['lambda'] 	= c['Dl2reg']#1e-4
        wss = [1, 2, 3, c['spad']]

        setence_model = SentenceModelingLayer(conf, sentences, wss)
